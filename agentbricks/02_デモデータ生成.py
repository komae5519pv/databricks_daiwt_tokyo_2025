...
# DBTITLE 1,スキーマ配下ALL削除
# spark.sql(f"""
# DROP SCHEMA {MY_CATALOG}.{MY_SCHEMA} CASCADE;
# """)
...
# print(f"{MY_CATALOG}.{MY_SCHEMA}配下の全てのテーブルが削除されました。")
...
# ai_query実行フラグがTrueの場合のみ実行
if RUN_AI_QUERY_FLG:
    print("AI Query実行フラグがTrueなのでスキップします")
else:
...
if RUN_AI_QUERY_FLG:
    ...
    csv_path = f"/Volumes/{MY_CATALOG}/{MY_SCHEMA}/{MY_VOLUME}/feedbacks/gold_feedback.csv"  # ファイル名まで
    df = (
    ...
    df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(
        f"{MY_CATALOG}.{MY_SCHEMA}.gold_feedbacks"
    )
...
if RUN_AI_QUERY_FLG:
  ...
  spark.sql(f'''
  CREATE OR REPLACE TABLE gold_feedbacks
  SELECT
  ...
  print("顧客レビューデータの感情スコアリング・分類・要約が完了しました！")
    ...
    print("AI Query実行フラグがFalseなのでスキップします")
